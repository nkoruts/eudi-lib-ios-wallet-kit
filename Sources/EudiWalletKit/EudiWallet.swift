...
public final class EudiWallet: ObservableObject, @unchecked Sendable {
	...
	func prepareIssuing(id: String, docType: String?, displayName: String? = nil, keyOptions: KeyOptions? = nil, disablePrompt: Bool = false, promptMessage: String? = nil) async throws -> OpenId4VCIService {
	...
	func finalizeIssuing(issueOutcome: IssuanceOutcome, docType: String?, format: DocDataFormat, issueReq: IssueRequest, openId4VCIService: OpenId4VCIService) async throws -> WalletStorage.Document  {
		...
		case .issued(let data, let str, let cc):
            ...
            docTypeToSave = if format == .cbor, let data { IssuerSigned(data: [UInt8](data))?.issuerAuth.mso.docType ?? docTypeOrScope }
            else if format == .sdjwt, let str, let ds = str.data(using: .utf8) {
                // TODO: - Remove RPs issue
                if docTypeOrScope == EuPidModel.euPidDocType {
